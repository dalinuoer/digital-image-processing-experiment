...
%   and maximum values. The encode data is returned as a structure
%   Y:
%       Y.code      The Huffman-encoded values of X, stored in
%                   a uint16 vector. The other fields of Y contain
%                   additional decoding informaition, including:
...
%   codes of reasonable length. The maximum row or colum dimension
%   of X is 65535.
%
%   See also HUFF2MAT

if ndims(x) ~= 2 | ~isreal(x) | (~isnumeric(x) & ~islogical(x))
    error('X must be a 2-D real nemeric or logical matrix.');
end
